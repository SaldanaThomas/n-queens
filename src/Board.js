 ...
 */
    ...
    hasRowConflictAt: function(rowIndex) {
      const BOARDROW = this.attributes[rowIndex];
      let counter = 0;
      for (spot of BOARDROW) {
        if (spot) {
          counter++;
        }
        if (counter > 1) {
    ...
    hasMajorDiagonalConflictAt: function(majorDiagonalColumnIndexAtFirstRow) {
      ...
      for (let rowIndex = 0; rowIndex < BOARD.n; rowIndex++) {
    ...
    hasAnyMajorDiagonalConflicts: function() {
      const BOARD = this.attributes;
      for (let i = BOARD.n * -1 + 1; i < BOARD.n; i++) {
